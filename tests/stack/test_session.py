...
def test_coreg_resume_restores_burst_array_shape_from_samples(
    ...
    monkeypatch.setattr(
        "faninsar.missions.s1.processing.load_production_scene", restore_scene
    )
...
def test_publish_generation_exports_canonical_network_view(tmp_path: Path) -> None:
    ...
    try:
        ...
        from faninsar.timeseries import TimeSeries

        assert isinstance(nsbas_result, TimeSeries)
    finally:
        generation.close()
