...
from faninsar.processing.coreg import resample_complex_deramped_reramp
...
def test_workflow_stage_interferogram_goldstein_zero_no_nameerror() -> None:
    """workflow.stage_interferogram(alpha=0) must not reference undefined filtered."""
    ...
    carrier = TOPSCarrierModel(
        ...
        range_sampling_rate_hz=64e6,
        azimuth_time_interval_s=0.002,
        doppler_centroid_hz=(0.0,),
        doppler_t0_s=0.0,
        fm_rate_hz_s=(-2300.0,),
        fm_t0_s=0.0,
        burst_sensing_time_s=0.0,
    ...
    ref.carrier = carrier
    sec.carrier = carrier
    state = PairWorkflowState(pair_id="t", reference=ref, secondary=sec)
    rng = np.random.default_rng(1)
    state.reference_deramped = (
        rng.normal(size=(24, 40)) + 1j * rng.normal(size=(24, 40))
    ).astype(np.complex64)
    state.secondary_aligned = state.reference_deramped * np.exp(1j * 0.05)
    out = stage_interferogram(state, multilook=(2, 4), goldstein_alpha=0.0)
    assert out.wrapped_phase is not None
    assert out.complex_ifg is not None
    np.testing.assert_allclose(out.wrapped_phase, np.angle(out.complex_ifg))
