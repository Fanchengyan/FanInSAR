...
def test_ampcor_magnitude_tile_cyclic_shift_matches_rolled_source() -> None:
    ...
    from faninsar.processing.coregistration import offsets as offsets_mod
...
def test_explicit_compile_rejects_partial_final_batch_before_dispatch() -> None:
    ...
    pytest.importorskip("torch")
    from faninsar.processing.coregistration.ampcor_backend import (
...
def test_public_compile_candidate_pads_partial_batch_without_recompile(
    ...
    from faninsar.processing.coregistration.ampcor_backend import prepare_ampcor_compile
...
def test_full_iw1_shape_rejects_overlapping_view(
    ...
    pytest.importorskip("torch")
    from faninsar.processing.coregistration import geometry_coreg
    ...
    # The production pre-roll admission charges the same bounded tile budget.
    monkeypatch.setattr(geometry_coreg.np, "roll", lambda array, **_: array)
    monkeypatch.setattr(
        geometry_coreg,
        "estimate_patch_amplitude_shift",
    ...
    with pytest.raises(
        ...
        match=r"overlapping|out-of-bounds|only float32 or complex64",
    ):
        estimate_patch_amplitude_shift(
            oversized,
            oversized,
...
def test_ampcor_policy_explicit_cpu_stays_cpu(
    ...
    pytest.importorskip("torch")
    from faninsar.processing.coregistration import resolve_ampcor_policy
...
def test_ampcor_candidate_energy_guard_uses_fft_fallback(
    ...
    torch = pytest.importorskip("torch")
    from faninsar.processing.coregistration import offsets as offsets_mod
    from faninsar.processing.coregistration.ampcor_backend import AmpcorEnergyCandidate
