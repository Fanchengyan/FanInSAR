"""Fixed, opt-in live remote transfer verifier.

The transfer verifier is inert unless ``FANINSAR_RUN_LIVE_REMOTE_TESTS=1`` is
set.  Deterministic discovery checks remain available without network access.
Live payloads and evidence are written only below the caller-selected
temporary root.
"""
...
_LIVE_ENABLED = os.environ.get("FANINSAR_RUN_LIVE_REMOTE_TESTS") == "1"
...
_ROOT = Path(
    os.environ.get(
        "FANINSAR_LIVE_REMOTE_TEST_OUTPUT_DIR", f"/private/tmp/live-remote-{_COMMIT}"
    )
)
    ...
    def install(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def ledger_response_bytes(ledger: remote._CallLedger, count: int) -> None:
            ...
            if operation is not None:
                operation["response_bytes"] = operation.get("response_bytes", 0) + count
...
def _register_lanes() -> tuple[tuple[str, str | None, str, str], ...]:
    ...
    register_pc_catalog("live-remote-pc")
    return (
        ("ASF SAFE", _ASF_GRANULE, _ASF_ITEM, "live-remote-asf"),
        ("LP DAAC NASADEM", _LPDAAC_GRANULE, _LPDAAC_ITEM, "live-remote-lpdaac"),
        ("Planetary Computer DEM", None, _PC_ITEM, "live-remote-pc"),
    )
                        ...
                        "links": [
                            {
                                "rel": "http://esipfed.org/ns/fedsearch/1.1/data#",
                                "href": (
                                    "https://datapool.asf.alaska.edu/data/item.zip"
                                ),
                            }
                        ],
                    }
...
@pytest.mark.skipif(
    not _LIVE_ENABLED,
    reason=("set FANINSAR_RUN_LIVE_REMOTE_TESTS=1 for the opt-in live verifier"),
)
def test_live_remote_fixed_full_transfers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for lane, concept_id, item_id, catalog in registrations:
        ...
        try:
            ...
            lanes.append(_lane_result(lane, item_id, selected, destination, recorder))
        except Exception as error:
    ...
    assert any(
        lane.get("final_origin") not in _REGISTERED_TERMINAL_ORIGINS for lane in lanes
    ), report
