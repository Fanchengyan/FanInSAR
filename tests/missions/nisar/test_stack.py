...
def test_nisar_stack_builds_shared_catalog_and_pair_topology(
    ...
    monkeypatch.setattr(
        "faninsar.missions.nisar.stack.NisarSensor.open_product",
        lambda _sensor, uri, **_kwargs: handles[Path(uri)],
    )
    calls: list[dict[str, object]] = []
    monkeypatch.setattr(
        "faninsar.missions.nisar.stack.NisarSensor.to_slc_product",
...
def test_nisar_stack_passes_explicit_admission_metadata_to_reader(
    ...
    monkeypatch.setattr(
        "faninsar.missions.nisar.stack.NisarSensor.open_product", open_product
    )
    monkeypatch.setattr(
        "faninsar.missions.nisar.stack.NisarSensor.to_slc_product",
...
def test_nisar_stack_fails_closed_before_shared_s1_processing(
    ...
    monkeypatch.setattr(
        "faninsar.missions.s1.processing.open_safe_product",
        fail_if_safe_opened,
    )
