...
from faninsar.processing.geometry.coordinates import GeoGrid
...
from faninsar.missions.nisar import NISARStack
from faninsar.missions.nisar.provider import (
...
from .test_stack import _result
...
def _stack(
    ...
    *,
    ...
    monkeypatch.setattr(
        "faninsar.missions.nisar.provider._geometry_shared_radar_window",
...
def test_nisar_provider_promotes_full_scene_without_bounded_window(
    ...
    monkeypatch.setattr(
        "faninsar.missions.nisar.provider._dense_secondary_mapping",
        lambda _reference, _secondary, bounds, **_kwargs: _identity_dense_mapping(
            bounds
        ),
    )
    monkeypatch.setattr(
        "faninsar.missions.nisar.provider._lanczos_source_coverage",
...
def test_nisar_three_date_multitile_radar_and_projected_geo_lifecycle(
    ...
    monkeypatch.setattr(
        "faninsar.missions.nisar.provider._geo_tile_for_radar_crop",
    ...
    monkeypatch.setattr(
        "faninsar.missions.nisar.provider._geocode_aligned_radar_tile",
