...
date-only :class:`~faninsar.core.pair.Pair` topology.  These records are
...
# Descriptive alias used by manifest-oriented callers.
NetworkProductRecord = NetworkProduct


@dataclass(frozen=True, slots=True)
class Interferogram:
    """Immutable result bundle for one existing :class:`~faninsar.core.pair.Pair`.
    ...
    def from_pair_name(
        ...
        *,
        ...
        wrapped_phase: Any | None = None,
        coherence: Any | None = None,
        unwrapped_phase: Any | None = None,
        masks: Mapping[str, Any] | None = None,
        metadata: Mapping[str, object] | None = None,
    ) -> Self:
        """Construct an interferogram from a canonical date-pair name.

        Parameters
        ----------
        pair_name : str
            Pair identifier accepted by :meth:`faninsar.core.pair.Pair.from_name`.
        samples, wrapped_phase, coherence, unwrapped_phase : array-like, optional
            Scientific representations for this pair.
        masks, metadata : mapping, optional
            Additional validity masks and immutable metadata.

        Returns
        -------
        Interferogram
            Product carrying the parsed :class:`~faninsar.core.pair.Pair` lineage.

        Raises
        ------
        InvalidProcessingStateError
            If ``pair_name`` cannot be parsed as two dates.
        ...
        try:
            ...
            pair = Pair.from_name(pair_name)
        except (TypeError, ValueError, IndexError) as error:
            reject_invalid_state(f"invalid interferogram pair name: {pair_name!r}")
            raise AssertionError("unreachable") from error
        return cls(
            pair=pair,
            complex_phase=samples,
            wrapped_phase=wrapped_phase,
            coherence=coherence,
            unwrapped_phase=unwrapped_phase,
    ...
    def pair_id(self) -> str:
        """Return the canonical pair identifier for this product."""
        return self.pair.name

    @property
    def primary_id(self) -> str:
        """Return the primary acquisition identifier."""
        return self.pair.primary_string()

    @property
    def secondary_id(self) -> str:
        """Return the secondary acquisition identifier."""
        return self.pair.secondary_string()

    @property
    def unwrapped(self) -> NDArray | None:
        """Return the unwrapped phase representation."""
        return self.unwrapped_phase

    @property
    def samples(self) -> NDArray | None:
        """Return complex samples under the processing seam alias."""
    ...
    def physical(self) -> Any:
        """Return the physical product type represented by this interferogram."""
        from faninsar.core.physical import PhysicalType

        if self.unwrapped_phase is not None:
            return PhysicalType.PHASE_UNWRAPPED
        return PhysicalType.IFG_COMPLEX
