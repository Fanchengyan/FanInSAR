...
Device policy matches :mod:`faninsar.processing.runtime.device`: ``"auto"``
resolves from hardware visible to the current process, preferring CUDA and
otherwise using CPU. Distributed callers pass ``"cuda"`` explicitly after
selecting a trusted GPU worker. On CPU the kernels compute in
float64/complex128; accelerator precision is selected per kernel from
qualified parity evidence.
"""
...
def carrier_phase_at_points_torch(
    ...
    *,
    ...
    Numerically mirrors
    :func:`faninsar.processing.coregistration.tops.deramp.carrier_phase_at_points`
...
def esd_azimuth_shift_torch(
    ...
    *,
    ...
    for col_start in range(0, n_range, range_chunk_size):
        ...
        diff_ifg = (look_ref_lower * look_sec_lower.conj()) * (
            look_ref_upper * look_sec_upper.conj()
        ).conj()
        amp = diff_ifg.abs()
        valid = amp > 0
        if bool(valid.any()):
