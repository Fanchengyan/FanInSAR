...
__global__ void rdr2geo_tcn_kernel(
    const double* target_range, const double* height_seed,
    const double* satellite, const double* velocity, int64_t count,
    const double* dem, int64_t dem_rows, int64_t dem_columns,
    double dem_x_start_deg, double dem_y_start_deg, double dem_dx_deg,
    double dem_dy_deg, double reference_height, double wavelength,
    double range_tolerance, double doppler_tolerance, int64_t primary_iter,
    int64_t budget, double look_sign, double* latitude_deg,
    double* longitude_deg, double* output_height, bool* converged,
    double* range_residual, double* doppler_residual, int32_t* iterations) {
  const int64_t point = static_cast<int64_t>(blockIdx.x) * blockDim.x +
                        threadIdx.x;
  if (point >= count) return;
  const double target = target_range[point];
  const double sat[3] = {satellite[point * 3], satellite[point * 3 + 1],
                         satellite[point * 3 + 2]};
  const double vel[3] = {velocity[point * 3], velocity[point * 3 + 1],
                         velocity[point * 3 + 2]};
  if (!(target > 0.0) || !isfinite(target) || !isfinite(wavelength) ||
      wavelength <= 0.0 || !isfinite(sat[0]) || !isfinite(sat[1]) ||
      !isfinite(sat[2]) || !isfinite(vel[0]) || !isfinite(vel[1]) ||
      !isfinite(vel[2])) return;

  double satellite_norm = sqrt(sat[0] * sat[0] + sat[1] * sat[1] +
                               sat[2] * sat[2]);
  double speed = sqrt(vel[0] * vel[0] + vel[1] * vel[1] + vel[2] * vel[2]);
  if (!(satellite_norm > 0.0) || !(speed > 0.0) || !isfinite(satellite_norm) ||
      !isfinite(speed)) return;
  ...
  double height = isfinite(height_seed[point]) ? height_seed[point]
                                                : reference_height;
  double old_llh[3] = {0.0, 0.0, 0.0};
  ...
  for (int64_t iteration = 0; iteration < budget && active; ++iteration) {
    if (!isfinite(height) || !(ellipsoid_height - height < target)) break;
    ...
    const double beta = look_sign *
        sqrt(fmax(0.0, target * sin_theta * target * sin_theta - alpha * alpha));
    double target_xyz[3];
    for (int axis = 0; axis < 3; ++axis) {
    ...
    double dot = 0.0;
    for (int axis = 0; axis < 3; ++axis) dot += vel[axis] * look[axis] / slant_range;
    latest_range_residual = slant_range - target;
    latest_doppler_residual = 2.0 * dot / wavelength;
    ++attempts;
    const bool now_converged =
        isfinite(latest_range_residual) && isfinite(latest_doppler_residual) &&
    ...
    if (iteration >= primary_iter) {
      ...
      for (int axis = 0; axis < 3; ++axis) target_xyz[axis] =
          0.5 * (old_xyz[axis] + dem_xyz[axis]);
      double average_lat = 0.0, average_lon = 0.0, average_height = 0.0;
      ecef_to_llh(target_xyz, &average_lat, &average_lon, &average_height);
      old_llh[0] = average_lon;
      old_llh[1] = average_lat;
  ...
  const double beta = look_sign *
      sqrt(fmax(0.0, target * sin_theta * target * sin_theta - alpha * alpha));
  double final_xyz[3];
  for (int axis = 0; axis < 3; ++axis) final_xyz[axis] =
      sat[axis] + alpha * along_track[axis] + beta * cross_track[axis] +
      gamma * normal[axis];
  ...
  if (final_slant_range > 0.0 && isfinite(final_slant_range)) {
    for (int axis = 0; axis < 3; ++axis)
...
std::vector<Tensor> rdr2geo_tcn_cuda_v2(
  ...
  check_cuda_vector(target_range_m, "target_range_m");
  check_cuda_vector(height_seed_m, "height_seed_m");
  const int device = target_range_m.get_device();
  check_cuda_matrix(satellite_position_m, "satellite_position_m",
                    target_range_m.numel(), device);
  check_cuda_matrix(satellite_velocity_m_s, "satellite_velocity_m_s",
                    target_range_m.numel(), device);
  check_same_device(target_range_m, height_seed_m, "height_seed_m");
  TORCH_CHECK(target_range_m.numel() == height_seed_m.numel(),
              "target_range_m and height_seed_m must have equal lengths");
  ...
  TORCH_CHECK(dem_height_m.scalar_type() == torch::kFloat64 &&
              ...
              "dem_height_m must be a contiguous float64 2-D tensor or empty scalar");
  TORCH_CHECK(std::isfinite(dem_dx_deg) && std::isfinite(dem_dy_deg) &&
              dem_dx_deg != 0.0 && dem_dy_deg != 0.0,
              "DEM spacing must be finite and non-zero");
  TORCH_CHECK(std::isfinite(reference_height_m) &&
              std::isfinite(wavelength_m) && wavelength_m > 0.0,
              "DEM reference height and wavelength must be finite; wavelength positive");
  check_solver_scalars(max_iter, extra_iter, range_tolerance_m,
                       "range_tolerance_m");
  TORCH_CHECK(std::isfinite(doppler_tolerance_hz) && doppler_tolerance_hz > 0.0,
  ...
  c10::cuda::CUDAGuard guard(target_range_m.device());
  const auto options = target_range_m.options();
  const int64_t count = target_range_m.numel();
  ...
  auto height = torch::full({count}, std::numeric_limits<double>::quiet_NaN(), options);
  ...
  if (count > 0) {
    ...
    rdr2geo_tcn_kernel<<<blocks, threads, 0, stream.stream()>>>(
        target_range_m.data_ptr<double>(), height_seed_m.data_ptr<double>(),
        satellite_position_m.data_ptr<double>(), satellite_velocity_m_s.data_ptr<double>(),
        count, dem_ptr, dem_rows, dem_columns, dem_x_start_deg, dem_y_start_deg,
        dem_dx_deg, dem_dy_deg, reference_height_m, wavelength_m,
        range_tolerance_m, doppler_tolerance_hz, max_iter,
        max_iter + extra_iter, right_looking ? 1.0 : -1.0,
        latitude.data_ptr<double>(), longitude.data_ptr<double>(),
        height.data_ptr<double>(), converged.data_ptr<bool>(),
...
std::vector<Tensor> rdr2geo_cuda_v2(
  ...
  return rdr2geo_tcn_cuda_v2(
      target_range_m, height_seed_m, satellite_position_m,
      satellite_velocity_m_s, dem_height_m, dem_x_start_deg, dem_y_start_deg,
      dem_dx_deg, dem_dy_deg, reference_height_m, wavelength_m,
