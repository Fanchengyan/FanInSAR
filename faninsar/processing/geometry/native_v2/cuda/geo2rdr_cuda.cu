...
__global__ void geo2rdr_kernel(
    ...
    int64_t budget, double time_tolerance, double doppler_tolerance,
    double* azimuth, double* range, bool* converged, double* range_residual,
    double* doppler_residual, int32_t* iterations) {
  const int64_t point = static_cast<int64_t>(blockIdx.x) * blockDim.x +
                        threadIdx.x;
  if (point >= count || !isfinite(latitude[point]) ||
      !isfinite(longitude[point]) || !isfinite(height[point])) return;
    ...
    if (isfinite(step) && fabs(step) < time_tolerance) {
      if (time >= orbit_start && time <= orbit_end) {
        ...
        if (final_range > 0.0 && isfinite(final_range)) {
          ...
          residual_doppler = final_doppler;
          solved = isfinite(final_doppler) &&
                   fabs(final_doppler) <= doppler_tolerance;
        }
      }
      if (solved) break;
    }
...
std::vector<Tensor> geo2rdr_cuda_v2(
  ...
  check_cuda_matrix(orbit_velocities_m_s, "orbit_velocities_m_s",
                    orbit_times_s.numel(), device);
  TORCH_CHECK(azimuth_time_interval_s > 0.0 && range_spacing_m > 0.0,
  ...
  TORCH_CHECK(std::isfinite(doppler_tol_hz) && doppler_tol_hz > 0.0,
  ...
  if (count > 0) {
    ...
    geo2rdr_kernel<<<blocks, threads, 0, stream.stream()>>>(
        ...
        time_tol_s, doppler_tol_hz, azimuth.data_ptr<double>(),
